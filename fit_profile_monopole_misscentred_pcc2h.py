...
if 'True' in args.boost:
    ...
    if 'True' in args.yan:
        ymiss      = True
        outfile = 'monopole_2h_ymiss_boost_'+file_name[:-4]+'out'
        tau = 0.3
    elif 'False' in args.yan:
        ymiss     = False
        outfile = 'monopole_2h_boost_'+file_name[:-4]+'out'
        tau = 0.4
elif 'False' in args.boost:
    boost = False
    if 'True' in args.yan:
        ymiss      = True
        outfile = 'monopole_2h_ymiss_'+file_name[:-4]+'out'
        tau = 0.3
    elif 'False' in args.yan:
        ymiss     = False
        outfile = 'monopole_2h_'+file_name[:-4]+'out'
        tau = 0.4
...
def log_likelihood(data_model, r, Gamma, e_Gamma):
    ...
    model = model_Gamma(multipoles,'t', misscentred = True, pcc = pcc) + two_halo
    sigma2 = e_Gamma**2
